...
func on_ability_upgrade_added(ability_upgrade: AbilityUpgrade, curren_upgrades: Dictionary):
	if ability_upgrade is Ability:
		var ability = ability_upgrade as Ability
		abilities.add_child(ability.ability_controller_scene.instantiate())
	elif ability_upgrade.id == "player_speed":
		velocity_component.max_speed = base_speed + (base_speed * curren_upgrades["player_speed"]["quantity"] * 0.1)
